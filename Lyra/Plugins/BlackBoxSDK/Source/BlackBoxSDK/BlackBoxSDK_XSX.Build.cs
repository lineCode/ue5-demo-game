...
public class BlackBoxSDKPlatform_XSX : BlackBoxSDKPlatform
{
    ...
    public BlackBoxSDKPlatform_XSX(ReadOnlyTargetRules inTarget, string inPluginDir, string inEngineDirectory) : base(inTarget, inPluginDir, inEngineDirectory)
    {
        SetupXCurlDir();
        SetupXSAPIDir();
    }
    ...
    {
        return Path.Combine(PluginDir, "DLLs/x64/XSX");
    }

    private string GetLibsDirPath()
    {
        return Path.Combine(PluginDir, "Libs/x64/XSX");
    }
